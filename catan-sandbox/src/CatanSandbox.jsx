...
function TradePanel({ currentPlayer, onTrade, onClose, nodes }) {
  ...
  let footerText = "";
  if (!canTrade) {
    if (totalGive < requiredRatio) {
      footerText = `Select ${requiredRatio} to give`;
    } else if (totalGive === requiredRatio && totalReceive < 1) {
      footerText = "Select 1 to receive";
    } else if (totalGive === requiredRatio && totalReceive === 1) {
      footerText = "Not enough resources";
    }
  }
                ...
                >
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <Button
                      ...
                      disabled={
                        receiveAmounts[resource] >= 1 || totalReceive >= 1
                      }
                    >
                      +
                    </Button>
...
// Control Dock
// ============================================
function ControlDock({ onReroll, onEndTurn, onNewGame, onReset, onAutoPlay }) {
  return (
    <motion.div
    ...
    >
      ...
      >
        ...
        <div className="p-4 flex items-center justify-between text-xs text-slate-600">
          <span>
            Knights:{" "}
            <span className="text-slate-400">{player.knightsPlayed}</span>
          </span>
          <span>
            Deck: <span className="text-slate-400">{devCardDeck.length}</span>
          </span>
        </div>
      </motion.div>
    </motion.div>
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        type === "info" && "bg-slate-800/90 text-slate-50"
      )}
    >
      {type === "success" ? (
...
export default function CatanSandbox() {
  ...
  const pushLog = useCallback((entry) => {
    setActionLog((prev) => {
      const next = [
        ...prev,
        { ...entry, id: entry.id || `${Date.now()}-${prev.length}` },
      ];
  ...
  const applyStateFromServer = useCallback(
    (state) => {
      ...
      if (state.devCardDeck) setDevCardDeck(state.devCardDeck);
    },
    [setBoard]
  );
  ...
  const startLocalGame = useCallback(() => {
    ...
    const boardWithPlacements = placeInitialBuildings(generateBoard(), numPlayers);
    ...
    setWinner(null);
    setGameId(null);
  }, [numPlayers, playerConfigs, setBoard]);
  ...
  }, [API_BASE, numPlayers, playerConfigs, applyStateFromServer, startLocalGame]);
  ...
  const handleAutoPlay = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/games/${gameId}/auto-play`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (data.state?.winner) {
        ...
        setLastAction({
          type: "success",
          message: `${data.state.winner.name} wins!`,
          timestamp: Date.now(),
        });
        setTimeout(() => setLastAction(null), 3000);
      }
      ...
      while (safety < 6) {
        ...
        if (data.action?.reason) {
          setLastAction({
            ...
            }: ${data.action.action} – ${data.action.reason}`,
            timestamp: Date.now(),
          });
        ...
        pushLog({
          ...
          message: `${
            ...
            data.action?.reason ? ` – ${data.action.reason}` : ""
          }`,
          turn: state?.turn || turn,
        });
  ...
  // ============================================
  // Local board seeding for initial buildings
  // ============================================
  function placeInitialBuildings(newBoard, numPlayersLocal) {
    ...
    const availableNodes = nodes.filter((n) => {
      ...
      return !n.adjHexes.some((hexIdx) =>
        desertTileIndices.includes(hexIdx)
      );
    });

    for (let playerId = 0; playerId < numPlayersLocal; playerId++) {
  ...
  const buyDevCard = () => {
    if (gameId) sendAction("buyDevCard");
  };
  ...
  const playDevCard = (cardIndex) => {
    ...
    if (!card.canPlay) {
      setLastAction({
        type: "error",
        message: "Cannot play card bought this turn!",
        timestamp: Date.now(),
      });
    ...
    if (card.type === "victory") {
      setLastAction({
        type: "error",
        message: "Victory point cards are auto-counted!",
        timestamp: Date.now(),
      });
      setTimeout(() => setLastAction(null), 2000);
      return;
    ...
    if (card.type === "knight") {
      ...
      // TODO: prompt for resources
      setShowDevCardPanel(false);
    } else if (card.type === "monopoly") {
      // TODO: prompt for resource choice
      setShowDevCardPanel(false);
    }
  };
